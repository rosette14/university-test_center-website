<html>
    ...
    <body>
        <div id='base' >
            ...
            <h1 >Role</h1>
            <br>
            <form  name="form1" action="<?php echo URL."roles/addRoleSave"?>" method="post" onsubmit="return validate()">
                <div class="input_field" id="input_name">
                    <input  type="text" name="role_name" id="role" placeholder="Role Name" required>
                </div>
                ...
                <div class="input_field">
                        <?php foreach($array as $row) {
                            if($row->permission_name!='Ask For Test'&& $row->permission_name!='See Students Results'
                            && $row->permission_name!='Take Test'&& $row->permission_name!='See Own Results'){
                            ?>
                            <div>
                           <input class="perm" type="checkbox" name=<?php echo $row->id ?> value="<?php echo $row->permission_name ?>"> <?php echo $row->permission_name ?>
                            </div>
                            <?php }} ?>
                </div>
                <div class="input_field">
